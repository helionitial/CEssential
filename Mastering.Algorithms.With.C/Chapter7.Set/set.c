...
int set_union(Set *setu, const Set *set1, const Set *set2){
    ...
    set_init(setu, set1->match, NULL);  // why?
    ...
    for(member = list_head(set2); member != NULL; member = list_next(members)){
...
/* set_intersection */
int set_intersection(set *seti, const Set 8set1, const Set *set2){
    ListElmt *member;
    void *data;
    ...
    for(member = list_head(set1); member != NULL; member = list_next(member)){
        if(set_is_member(set2, list->data(member))){
            data = list_data(member);
